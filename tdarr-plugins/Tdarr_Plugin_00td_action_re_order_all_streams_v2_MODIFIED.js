const details = () => ({
  id: 'Tdarr_Plugin_00td_action_re_order_all_streams_v2_MODIFIED',
  ...
  This plugin re-orders all streams based on: codecs,channels,languages,streamTypes,subtitle.
  `,
  Version: '1.00',
  Tags: 'action',
  Inputs: [
    {
      name: 'processOrder',
      type: 'string',
      defaultValue: 'codecs,channels,languages,subtitle,streamTypes',
      inputUI: {
        type: 'text',
      },
      tooltip:
...
The default order is suitable for most people.
        ...
        \\nExample:\\n
        codecs,channels,languages,subtitle,streamTypes
        `,
    },
    {
      name: 'languages',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  const {
    languages, codecs, channels, streamTypes, subtitle,
  } = inputs;
  ...
  const sortTypes = {
	...
	subtitle: {
      getValue: (stream) => {
        ...
        try {
          if (subMap[stream.tags.title]) {
            return subMap[stream.tags.title];
          }
        } catch (err) {
          // err
        }
