...
const details = () => {
  return {
    id: "Tdarr_Plugin_d5d3_iiDrakeii_FFMPEG_NVENC_Tiered_MKV_MODIFIED",
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  if (file.video_resolution === "480p" || file.video_resolution === "576p") {
    ...
    response.preset += `,${map} -dn -c:v hevc_nvenc -qmin 0 -cq:v 29 -b:v ${bitratetarget}k -maxrate:v 1500k -preset slow -rc-lookahead 32 -spatial_aq:v 1 -aq-strength:v 8 -a53cc 0 -c:a copy ${subcli}${maxmux}`;
    transcode = 1;
  }
  ...
  if (file.video_resolution === "720p") {
    ...
    response.preset += `,${map} -dn -c:v hevc_nvenc -qmin 0 -cq:v 30 -b:v ${bitratetarget}k -maxrate:v ${bitratemax}k -preset slow -rc-lookahead 32 -spatial_aq:v 1 -aq-strength:v 8 -a53cc 0 -c:a copy ${subcli}${maxmux}`;
    transcode = 1;
  }
  ...
  if (file.video_resolution === "1080p") {
    ...
    response.preset += `,${map} -dn -c:v hevc_nvenc -qmin 0 -cq:V 31 -b:v ${bitratetarget}k -maxrate:v ${bitratemax}k -preset slow -rc-lookahead 32 -spatial_aq:v 1 -aq-strength:v 8 -a53cc 0 -c:a copy ${subcli}${maxmux}`;
    transcode = 1;
  }
